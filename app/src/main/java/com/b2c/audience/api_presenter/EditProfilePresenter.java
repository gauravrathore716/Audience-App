...
public class EditProfilePresenter extends BasePresenter<IEditProfileView> {
    ...
    public void editprofile(final Context context, HashMap<String, RequestBody> map, @Part MultipartBody.Part imageFile) {

        getView().enableLoadingBar(context,true, context.getString(R.string.msg_please_wait));

        AppController.getInstance().getApiInterface().editSetting(map, imageFile).enqueue(new Callback<LoginRegisterResponse>() {
            @Override
            public void onResponse(@NonNull Call<LoginRegisterResponse> call, @NonNull Response<LoginRegisterResponse> response) {
                ...
                if (!handleError(response)) {
                    if (response.isSuccessful() && response.code() == 200) {
                        getView().onEditProfile(response.body());
                    }
                }else{
